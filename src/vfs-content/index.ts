...
export function loadFiles(vfs: VirtualFs) {
  ...
  addFile('types/_allTypes', FULL_Z3_TYPINGS, 'readonly');
  ...
  function addFile(
    ...
    filePath = `${filePath}.mjs`; // '*.mjs' to allow for global async
    vfs.createFile(filePath, content, mode);
  }

  function addTypesFile(filePath: string, defs: Z3TypeDefinition[]) {
    const contents = defs.map((def) => TYPES[def]);
    const content = contents.join('\n\n');
    return addFile(filePath, content, 'readonly');
  }
}
